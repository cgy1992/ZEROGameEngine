...
{
	ID3D11Buffer* dummy = nullptr;
	GraphicsSystem::Get()->GetContext()->GSSetConstantBuffers(slot, 1, &dummy);
}
...
void ConstantBuffer::UnbindCS(uint32_t slot) const
{
	ID3D11Buffer* dummy = nullptr;
	GraphicsSystem::Get()->GetContext()->CSSetConstantBuffers(slot, 1, &dummy);
}