...
{
	const FLOAT clearColor[4] = { 0.49f, 0.50f, 0.58f, 1.0f };
...
{
	ASSERT(mSwapChain != nullptr, "[GraphicsSystem] Failed to get swap chain buffer height.");
...
float GraphicsSystem::GetAspectRatio() const
{
	...
	float aspect = static_cast<float>(mSwapChainDesc.BufferDesc.Width) / static_cast<float>(mSwapChainDesc.BufferDesc.Height);
	return aspect;
}